...
import {
  formatConfidence,
  normalizePlan,
} from "../utils/planNormalizer";

interface RagPlanCardProps {
  plan: unknown;
  plantId?: string;
  savedPlanId?: string;
...
export function RagPlanCard({
  ...
  const { draft, setDraft } = usePlanReviewContext();
  const normalizedPlan = normalizePlan(plan, plantId);
      ...
      <View className="p-4">
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            ...
            router.push("/(main)/plan-review" as any);
          }}
        >
