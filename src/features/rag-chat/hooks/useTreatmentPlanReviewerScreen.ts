...
export function useTreatmentPlanReviewerScreen() {
  ...
  const submit = async () => {
    ...
    const schedulePayload: TreatmentPlanCreateRequest["schedule"] =
      schedule.map((event) => ({
    ...
    const payload: TreatmentPlanCreateRequest = {
    ...
    try {
      await createTreatmentPlan.mutateAsync(payload);
      clearDraft();
      router.replace({
  ...
  const isSubmitting = createTreatmentPlan.isPending;
