...
import { CATEGORY_DOT_COLORS, initCalendarLocale } from "../../plant-event/components/calendarConstants";
...
export type OfflinePlantEventHubScreenProps = {
  defaultView?: ViewType;
  hideFilter?: boolean;
  detailReturnTo?: string;
};

export function OfflinePlantEventHubScreen({
  defaultView = "month",
  hideFilter = false,
  detailReturnTo = "/(offline)/plant-events",
}: OfflinePlantEventHubScreenProps) {
  ...
  const isFiltered =
    !!(activeFilter.farmPlotId || activeFilter.farmZoneId || activeFilter.plantId ||
      activeFilter.targetType || activeFilter.eventType || activeFilter.selectedApplyId);
  const [isAddFormVisible, setIsAddFormVisible] = useState(false);
  const [initialEventForAdd, setInitialEventForAdd] = useState<any>(null);
  ...
  const calendarParams: CalendarParams = useMemo(
    () => ({
      ...(targetType === "FARM_PLOT"
        ? selectedId === "ALL_FARMS" ? {} : { farmPlotId: selectedId }
  ...
  const handleToggleComplete = (event: PlantEventResponse) => {
    updateEventMutation.mutateAsync(event.id).then(() => eventsQuery.refetch());
  };
  ...
  useEffect(() => {
    ...
    const first = { id: "ALL_FARMS", name: t("offline.allFarms", "Tất cả các vườn") };
  ...
  const handleAddEvent = () => {
    const dateParam = activeView === 'week' ? selectedWeekDate : selectedMonthDate;
    setInitialEventForAdd({
      calculatedStartDate: dateParam ? `${dateParam}T00:00:00` : undefined,
      calculatedEndDate: dateParam ? `${dateParam}T00:00:00` : undefined,
      farmPlotId: targetType === 'FARM_PLOT' && selectedId !== 'ALL_FARMS' ? selectedId : undefined,
  ...
  return (
    <View className="flex-1 bg-slate-50 dark:bg-slate-950">
      ...
      {!hideFilter && selectedId && (
        <PlantEventHubFilterBar
          ...
          data={{
            applies: [],
            farmPlots: [
              { id: "ALL_FARMS", name: t("offline.allFarms", "Tất cả các vườn") },
              ...farmPlots.map((p) => ({ id: p.id, name: p.name })),
            ],
            plants,
      ...
      <OfflinePlantEventFormScreen
        ...
        event={initialEventForAdd}
        onClose={() => setIsAddFormVisible(false)}
        onSaved={() => eventsQuery.refetch()}
      />
    </View>
  );
}

export default OfflinePlantEventHubScreen;
