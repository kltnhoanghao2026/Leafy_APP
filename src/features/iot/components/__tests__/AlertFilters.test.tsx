...
describe("AlertFilters", () => {
  ...
  it("uses picker labels and keeps raw IDs out of the main filter UI", () => {
    ...
    expect(screen.queryByPlaceholderText("Enter device identifier manually")).toBeNull();
  });
  ...
  it("keeps manual technical inputs behind advanced filters", () => {
    ...
    expect(screen.getByPlaceholderText("Enter device identifier manually")).toBeTruthy();
    expect(screen.getByPlaceholderText("Enter zone identifier manually")).toBeTruthy();
  });
});
